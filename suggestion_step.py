...
class Z3Model:
    ...
    def add_initial_constraints(self):
        ...
        for i in range(self.automaton.getNbTransition()):
            obs = f"isObservable_{i + 1}"
            self.observable_map[obs] = i  # Store mapping
    ...
    def printAutomatonInfo(self):
        ...
        print("delta:", self.delta)
        print("Observable map:", self.observable_map)
    ...
    def print_unsat_core(self):
        ...
        for constraint in unsat_core:
            ...
            if constraint_str in self.observable_map:
                transition_index = self.observable_map[constraint_str]
                transition = self.automaton.getTransitionAt(transition_index)
                event_id = transition.getEventId()
                core_analysis[constraint_str] = event_id
            else:
                core_analysis[constraint_str] = "Unknown"
        
        for key, value in core_analysis.items():
            print(f"{key} -> {value}")
        
        self.suggestions_based_on_unsat_core(core_analysis)

    def suggestions_based_on_unsat_core(self, core_analysis):
        suggestions = set()  # Use a set to avoid duplicate suggestions
        for constraint_str, event_id in core_analysis.items():
            if "length_fp" in constraint_str or "length_np" in constraint_str:
                ...
                suggestions.add(f"Review the event corresponding to constraint {constraint_str} (Event ID: {event_id})")
            else:
